def momentum_strategy(engine, prices, timestamp):
    ...
    vol_window  = 20
    gross       = 1.0   # share of capital committed
    rebal_every = 5
    min_resize  = 0.01
    adaptive    = False
    verbose     = True
    ...
    # sqrt(vol) leaves most of the weight on momentum while still penalising wild names
    ...
    # Shorts tie up balance the same as longs, so count both sides at absolute value.
    ...
    if verbose:
        print("bar %4d  %s  L=%s  S=%s  bal=%.0f"
              % (n, str(timestamp)[:10], ",".join(longs), ",".join(shorts), engine.balance))

    # The engine accepts only ONE order per ticker per timestamp, so a flip crosses
    # through zero in a single order rather than a close followed by an open.
    orders = []
    ...
    # Reduce exposure first so the freed balance is available for the new positions.
    ...
    for ticker, delta, growth in orders:
        ...
        if growth > 0:
            ...
            if affordable < volume:
                print("  short of balance on %s: wanted %d, took %d" % (ticker, volume, affordable))
...
def _pick(order, holding, k, buffer_):
    """Pick k names, letting something we already own keep its slot if it's still close."""
