def momentum_strategy(engine, prices, timestamp):
    ...
    vol_target  = False # cut exposure when vol spikes. UNTESTED on real data - synthetic
                        # says roughly neutral, but synthetic has mispredicted the
                        # direction of every change so far. Treat as a gamble.
    vol_floor   = 0.35  # never shrink below this fraction of gross
    quality_window = 120
    quality_power  = 0.5   # 0 = v8 behaviour ($3.13M, 7.13% DD)
                           # 1 = v9 behaviour ($2.42M, 4.83% DD)
                           # 0.5 interpolates between two REAL measurements
    ...
    # Volatility targeting. Quality weighting buys drawdown protection by discarding
    # signal - it changes WHICH names you hold. This changes HOW MUCH you hold instead:
    # when the average name is more volatile than its own long-run norm, shrink the book
    # by that ratio. Capped at 1.0 because the balance constraint means we can never
    # lever above full deployment anyway, and floored so a volatile stretch cannot flatten
    # the book entirely.
    if vol_target:
        recent_vol = float(np.nanmean(vol.to_numpy(dtype=float)))
        long_vol = float(np.nanmean(prices.pct_change().std().to_numpy(dtype=float)))
        if recent_vol > 0 and long_vol > 0:
            gross = gross * max(vol_floor, min(1.0, long_vol / recent_vol))
