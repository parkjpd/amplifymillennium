def momentum_strategy(engine, prices, timestamp):
    ...
    beta_window = 60
    rev_weight  = 0.0   # how hard to lean AGAINST recent moves (0 = pure momentum)
    rev_window  = 10
    verbose     = True
    ...
    # Long horizons trend, short horizons snap back - the regime table shows both at once.
    # The long-horizon `mom` above only harvests the first. Subtracting a standardised
    # short-horizon return bets against the second instead of ignoring it. Both legs are
    # standardised cross-sectionally so `rev_weight` means the same thing regardless of
    # how wide the two return distributions happen to be.
    if rev_weight and n > rev_window + 1:
        recent = (prices.iloc[-1] / prices.iloc[-1 - rev_window]) - 1.0
        recent = (recent / np.sqrt(vol)).dropna()
        common = score.index.intersection(recent.index)
        if len(common) >= 2 * k:
            a = score[common]
            b = recent[common]
            sa, sb = float(a.std()), float(b.std())
            if sa > 0 and sb > 0:
                score = (a - a.mean()) / sa - rev_weight * ((b - b.mean()) / sb)
